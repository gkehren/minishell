...
void		child_process_builtins(int *fd, int *fdd, t_exec *exec,
				t_list *lcmd);
int			only_builtins(t_builtins p_builtins, t_cmd *cmd,
				t_list *venv, t_exec *exec);
...
int			set_error_files(char **error, t_token_lex **tmp_token_error,
				char *errno_str, t_token_lex *tmp_token);

/*						HEREDOC						*/
t_heredoc	init_heredoc_var(char *stop, int heredoc_ex, t_files *files,
				int mode);
